...
@Tag(name = "Tipos de Combustível", description = "Endpoints para gerenciar os tipos de combustível")
public class TipoCombustivelController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Dados de entrada inválidos ou regra de negócio violada",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(oneOf = {ErrorResponseDTO.class, TipoCombustivelRequestDTO.class})))
    })
    @PostMapping("/{id}")
    @ResponseStatus(HttpStatus.CREATED)
    public TipoCombustivelResponseDTO create(@Valid @RequestBody TipoCombustivelRequestDTO requestDTO) {
    ...
    @PutMapping
    public TipoCombustivelResponseDTO update(@PathVariable Long id, @Valid@RequestBody TipoCombustivelRequestDTO requestDTO) {
        return service.update(id, requestDTO);
    }

    @Operation(summary = "Deleta um tipo de combustível por ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "Combustível deleatdo com sucesso"),
            @ApiResponse(responseCode = "404", description = "Combustível não encontrado")
    })
    @DeleteMapping("/{id}")
